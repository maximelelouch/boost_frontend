...
const MENU_ITEMS = [
  { icon: Users, label: "Ami(e)s", color: "text-blue-500" },
    ...
    <aside className="fixed left-0 top-14 h-[calc(100vh-56px)] w-[300px] p-4 overflow-y-auto hidden xl:block hover:overflow-y-scroll scrollbar-hide">
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-3 p-2 hover:bg-gray-200 rounded-lg cursor-pointer transition">
          ...
          <span className="font-semibold text-sm">Antoine Emmanuel ESSOMBA</span>
        </div>

        {/* Liste Menu */}
        {MENU_ITEMS.map((item, index) => (
          <div key={index} className="flex items-center gap-3 p-2 hover:bg-gray-200 rounded-lg cursor-pointer transition">
